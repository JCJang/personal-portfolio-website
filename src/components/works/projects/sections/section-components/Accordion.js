...
const Accordion = ({m, l, title="", sectionTitles=[], sectionResultsArr=[], sectionApplicationArr=[], problemSolution=false
}) => {
  ...
  <div className="subtitle1 transition selfCentered" style={{fontSize:"3.3rem",whiteSpace: "nowrap", width:"20vw",transform:"rotate(-90deg)"}}>{problemSolution?"Finding Solutions":"Results"}</div>
        ...
        <div className="Column">
            <div>{sectionTitles.map((title, index)=>{
                return <AccordionLabel m={m} title={title} focus={focus} setFocus={setFocus} problemSolution={problemSolution} sectionResults={sectionResultsArr[index]} sectionApplication={sectionApplicationArr[index]}/>})}
              </div>
        </div>
    </div>
